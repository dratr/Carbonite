...
--	end
	...
	self.WrathFlyName = C_Spell.GetSpellName(54197)
	self.AzerothFlyName = C_Spell.GetSpellName(90267)
	self.PandariaFlyName = C_Spell.GetSpellName(115913)
	self.DraenorFlyName = C_Spell.GetSpellName(191645)
	self.LegionFlyName = C_Spell.GetSpellName(233368)
end
...
function Nx.Travel:MakePath (tracking, srcMapId, srcX, srcY, dstMapId, dstX, dstY, targetType)
	...
	if not srcInfo or not dstInfo then
		return
	end
...
function Nx.Travel:GetRidingSkill()
	local RidingSpells = {
		[10] = C_Spell.GetSpellInfo (33388), -- Apprentice Riding
		[75] = C_Spell.GetSpellInfo (33391), -- Journeyman Riding
		[150] = C_Spell.GetSpellInfo (34090), -- Expert Riding
		[300] = C_Spell.GetSpellInfo (90265), -- Master Riding
	}
	local SkillRiding = 0

	for skill, info in pairs (RidingSpells) do
		if info ~= nil and C_Spell.GetSpellInfo (info["name"]) ~= nil then
			SkillRiding = skill
			break
		end
	end
